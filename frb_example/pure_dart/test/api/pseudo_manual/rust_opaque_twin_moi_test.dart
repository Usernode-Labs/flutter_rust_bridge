...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  test('create and dispose', () async {
    var futureData = createOpaqueTwinMoi();
  ...
  test('simple call', () async {
    var opaque = await createOpaqueTwinMoi();
  ...
  test('create enums of opaque type', () async {
    ...
    (data[1] as EnumOpaqueTwinMoi_Primitive).field0.dispose();

    expect(await runEnumOpaqueTwinMoi(opaque: data[2]), "\"String\"");
    ...
    expect(
        ...
        "lifetime: \\\"static str\\\" "
        "})\"");
    (data[4] as EnumOpaqueTwinMoi_Mutex).field0.dispose();
    await expectLater(() => runEnumOpaqueTwinMoi(opaque: data[4]),
        throwsA(isA<DroppableDisposedException>()));
  });

  test('opaque field', () async {
    var data = await createNestedOpaqueTwinMoi();
  ...
  test('array', () async {
    var data = await opaqueArrayTwinMoi();
    await futurizeVoidTwinMoi(opaqueArrayRunTwinMoi(data: data));
    data[0].dispose();
  ...
  test('vec', () async {
    var data = await opaqueVecTwinMoi();
    await futurizeVoidTwinMoi(opaqueVecRunTwinMoi(data: data));
    data[0].dispose();
  ...
  //   data.move = true;
  //   expect(
  //       await unwrapRustOpaqueTwinMoi(opaque: data),
  //       "content - Some(PrivateData "
  //       "{"
  //       " content: \"content nested\", "
  //       "primitive: 424242, "
  ...
  //
  //   var data2 = await createOpaqueTwinMoi();
  //   await expectLater(() => unwrapRustOpaqueTwinMoi(opaque: data2),
